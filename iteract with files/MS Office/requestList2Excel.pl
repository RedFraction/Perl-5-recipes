...
sub createFile{
    ...
    {
    my $FILE_PATH = getcwd() . "/Cписок посещений" . localtime(time) . '.xls';
    ...
    #Форматы вывода
    my $dateformat = $workbook->add_format(bold => 1, align => 'center', shrink => 1);             #Дата
    my $brenchformat = $workbook->add_format(bold => 1, align => 'left', size => 12, shrink => 1); #Филиал
    ...
    foreach my $i (@branches){
        $worksheet->write($rowItr++, $colItr, $branchesIp{$i}, $brenchformat);
    }
